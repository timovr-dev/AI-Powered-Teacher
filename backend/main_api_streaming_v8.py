...
async def upload_pdf(request: Request, file: UploadFile = File(...)):
    try:
        ...
        async def stream_learning_plan():
            ...
            quiz = generate_quiz(learning_plan_chunks=learning_plan_chunks)
            user_dict[user_id]['quiz'] = quiz
            print("$$$$$ Quiz is generated $$$$$")
...
def get_quiz(request: Request):
    user_id = get_user_id(request=request)
    quiz = []
    if 'quiz' in user_dict[user_id]:
        quiz = user_dict[user_id]['quiz']
...
def generate_quiz(learning_plan_chunks):
    """
    this function uses ALLaM to generate quiz from the learning plan chucks
